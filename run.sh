...
#    GITHUB_USERNAME - name of my github user, e.g. RobertZruban
...
EOF
    ...
    git commit -m 'feat!: populated from repo template'

    # if GH_TOKEN is set, set the remote url to it
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://$GITHUB_USERNAME:$GH_TOKEN@github.com/RobertZruban/$REPO_NAME"
    fi
    git push origin "$UNIQUE_BRANCH_NAME"

    # open a PR from the feature branch into main
    gh pr create \
        --title "feat: populated from repo template" \
        --body "This PR was generated by repo " \
        --base main \
